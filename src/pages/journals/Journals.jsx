...
import {
  Add, ExpandMore, ExpandLess,
  RateReview, CheckCircle, Cancel,
  HourglassEmpty,
...
const JournalCard = ({ journal, currentUser, onAssign, onReview }) => {
  ...
  const statusMap = {
    pending: { label: t('journals.status.pending'), color: 'default', icon: <HourglassEmpty fontSize="small" /> },
    under_review: { label: t('journals.status.under_review'), color: 'warning', icon: <RateReview fontSize="small" /> },
    accepted: { label: t('journals.status.accepted'), color: 'success', icon: <CheckCircle fontSize="small" /> },
    rejected: { label: t('journals.status.rejected'), color: 'error', icon: <Cancel fontSize="small" /> },
  };
      ...
      <CardContent>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1.5 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box>
              <Typography fontWeight={600}>{journal.author?.name}</Typography>
              <Typography variant="caption" color="text.secondary">
                {journal.author?.specialty || t('journals.researcher')}
              </Typography>
            </Box>
          </Box>
        ...
        {journal.reviews?.length > 0 && (
          <Box sx={{ mt: 1.5, display: 'flex', gap: 1 }}>
            <Chip
              label={t('journals.reviewers', { count: journal.reviews.length })}
              size="small"
              variant="outlined"
        ...
        <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
          {currentUser?.role === 'admin' && journal.status === 'pending' && (
            <Button size="small" variant="outlined" onClick={() => onAssign(journal)}>
              {t('journals.assignReviewers')}
            </Button>
          )}

          {journal.reviews?.some((r) => r.reviewer_id === currentUser?.id && r.decision === 'pending') && (
            <Button size="small" variant="contained" startIcon={<RateReview />} onClick={() => onReview(journal)}>
...
const SubmitDialog = ({ open, onClose, onSubmit }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle fontWeight={700}>{t('journals.submitNewPaper')}</DialogTitle>
      <DialogContent>
        {error && (
        ...
        <TextField
          ...
          rows={5}
          dir={textDir}
          helperText={t('journals.charMinAbstract', { count: form.abstract.length })}
        />
      </DialogContent>
      <DialogActions sx={{ p: 2 }}>
        ...
        <Button variant="contained" onClick={handleSubmit} disabled={loading}>
          {loading ? <CircularProgress size={20} /> : t('journals.submitPaper')}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
useEffect(() => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      <DialogTitle fontWeight={700}>{t('journals.assignTitle')}</DialogTitle>
      <DialogContent>
        ...
        <FormControl fullWidth>
          ...
          <Select
            ...
            renderValue={(sel) => sel.map((id) => users.find((u) => u.id === id)?.name).join(', ')}
          >
            {users.map((u) => (
              <MenuItem
                key={u.id}
              ...
              >
                {u.name} — {u.specialty || t('journals.researcher')}
              </MenuItem>
            ))}
          </Select>
        </FormControl>
      ...
      <DialogActions sx={{ p: 2 }}>
        ...
        <Button variant="contained" onClick={handleAssign} disabled={loading || selected.length === 0}>
          {loading ? <CircularProgress size={20} /> : t('journals.assignBtn')}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
const ReviewDialog = ({ open, journal, onClose, onSubmit }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle fontWeight={700}>{t('journals.reviewTitle')}</DialogTitle>
      <DialogContent>
        {error && (
          <Alert severity="error" sx={{ mb: 2 }}>
            {error}
        ...
        <FormControl fullWidth>
          ...
          >
            <MenuItem value="accept">{t('journals.decisionAccept')}</MenuItem>
            <MenuItem value="revision">{t('journals.decisionRevision')}</MenuItem>
            <MenuItem value="reject">{t('journals.decisionReject')}</MenuItem>
          </Select>
        </FormControl>
      </DialogContent>
      <DialogActions sx={{ p: 2 }}>
        ...
        <Button variant="contained" onClick={handleSubmit} disabled={loading}>
          {loading ? <CircularProgress size={20} /> : t('journals.sendReview')}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
const Journals = () => {
  ...
  const handleAssign = async (journalId, reviewerIds) => {
    const { data } = await axiosInstance.post(`/journals/${journalId}/assign-reviewers`, {
      reviewer_ids: reviewerIds,
    });
    setJournals((prev) => prev.map((j) => (j.id === journalId ? data.journal : j)));
  ...
  const toReview = journals.filter((j) => j.reviews?.some((r) => r.reviewer_id === user?.id));

  const tabData = [
    { label: t('journals.tabAll', { count: journals.length }), data: journals },
    { label: t('journals.tabMine', { count: myJournals.length }), data: myJournals },
    { label: t('journals.tabReview', { count: toReview.length }), data: toReview },
  ];
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<Add />} onClick={() => setSubmitOpen(true)}>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent sx={{ textAlign: 'center', py: 6 }}>
            <Typography color="text.secondary">{t('journals.emptyTab')}</Typography>
          </CardContent>
        </Card>
      ) : (
